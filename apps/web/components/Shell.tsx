...
export default function Shell({ children }: ShellProps) {
    ...
    return (
        <div className="flex h-screen bg-white dark:bg-[#020617] overflow-hidden">
            ...
            {/* Mobile Sidebar */}
            {!isAuthPage && (
                <aside className={`fixed inset-y-0 left-0 w-[300px] bg-white dark:bg-slate-950 border-r border-slate-200 dark:border-slate-800 z-[101] transform transition-transform duration-300 ease-in-out md:hidden flex flex-col ${isMobileMenuOpen ? 'translate-x-0' : '-translate-x-full'
                    }`}>
                            ...
                            <div className="grid grid-cols-2 gap-2">
                                {locales.map((cur) => (
                                    <button
                                        ...
                                        className={`flex items-center justify-center px-3 py-2 text-xs font-bold rounded-lg border transition-all ${currentLocale === cur
                        ...
                        {navItems.map((item) => {
                            ...
                            return (
                                <Link key={item.id} href={item.href} title={item.label}>
                                    <div className={`w-10 h-10 rounded-xl flex items-center justify-center transition-all duration-200 group relative ${isActive
                                        ? 'bg-blue-600 text-white shadow-lg shadow-blue-500/20'
            ...
            <main className="flex-1 flex flex-col min-w-0 relative">
                {/* Global Header */}
                {!isAuthPage && (
                    <header className="w-full h-[56px] flex items-center justify-between px-4 md:px-8 border-b border-slate-200/60 dark:border-slate-800/50 bg-white/30 dark:bg-[#020617]/30 backdrop-blur-lg saturate-150 absolute top-0 z-50">
                ...
                <div className={`flex-1 overflow-y-auto custom-scrollbar ${isAuthPage ? 'flex items-center justify-center bg-slate-50 dark:bg-slate-950' : ''}`}>
                    <div className={`${isAuthPage ? 'container mx-auto w-full p-4' : 'w-full min-h-full my-12 mx-auto'}`}>
                        {children}
                    </div>
                </div>
            </main>
