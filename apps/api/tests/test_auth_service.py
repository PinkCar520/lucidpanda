...
def test_notification_preferences(auth_service):
    ...
    # 2. Update prefs
    auth_service.update_notification_preferences(
        user_id,
        email_enabled=False,
        sms_enabled=True,
        subscribed_types=["trading_alerts"],
    )
...
def test_in_site_messages(auth_service):
    ...
    # 1. Create message
    msg = auth_service.create_in_site_message(
        user_id, "Welcome", "Welcome to LucidPanda"
    )
...
def test_audit_logging(auth_service):
    ...
    auth_service.log_audit(
        user_id, "TEST_ACTION", ip_address="1.2.3.4", details={"info": "test"}
    )
    ...
    test_log = next(
        log_entry for log_entry in logs if log_entry.action == "TEST_ACTION"
    )
