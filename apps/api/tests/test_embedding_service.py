...
class TestEmbeddingService:
    ...
    def test_encode_dashscope_success(self, mock_text_embedding, mock_dashscope):
        ...
        # Mock 配置
        with patch(
            "src.lucidpanda.services.embedding_service.settings"
        ) as mock_settings:
            ...
            mock_response.output = {"embeddings": [{"embedding": [0.1] * 768}]}
    ...
    @patch(
        "src.lucidpanda.services.embedding_service.EmbeddingService._encode_dashscope"
    )
...
class TestEmbeddingServiceRetry:
    ...
    @patch("src.lucidpanda.services.embedding_service.dashscope")
    @patch("src.lucidpanda.services.embedding_service.TextEmbedding")
    def test_dashscope_retry_on_failure(
        self, mock_text_embedding, mock_dashscope, mock_sleep
    ):
        """测试 DashScope 失败重试"""
        # Mock 配置
        with patch(
