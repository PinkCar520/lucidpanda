...
def test_llm_fallback(engine):
    ...
    pending_item = {
        "id": 1,
        "source_id": "http://test.com/2",
        "content": "Geopolitical tensions rise",
        "url": "http://test.com/2",
        "gold_price_snapshot": 1950.0,
        "fed_val": 0,
    }
    engine.db.get_pending_intelligence.return_value = [pending_item]
    engine.deduplicator.is_duplicate.return_value = False

    engine.primary_llm.generate_json_async = AsyncMock(
        side_effect=Exception("API Error")
    )
    engine.primary_llm.analyze_async = AsyncMock(side_effect=Exception("API Error"))
    engine.fallback_llm.analyze_async = AsyncMock(
        return_value={
            "summary": {"en": "Fallback"},
            "sentiment": {"en": "Bullish"},
            "urgency_score": 6,
            "sentiment_score": 0.5,
