...
class AsyncRichCrawler:
    ...
    def __init__(self, max_concurrent=10, timeout=12):
        ...
        self.client_limits = httpx.Limits(
            max_keepalive_connections=5, max_connections=max_concurrent
        )
            ...
            for attempt in range(2):  # 最多尝试 2 次
                try:
                    response = await client.get(
                        jina_url, headers=headers, timeout=self.timeout
                    )
                    if response.status_code == 200:
                        ...
                        logger.warning(
                            f"Jina Reader Rate Limited (429) for {url}, retrying..."
                        )
                        await asyncio.sleep(2)
                    else:
                        logger.warning(
                            f"Jina Reader error {response.status_code} for {url}"
                        )
                except Exception as e:
                    if attempt == 1:
                        logger.warning(
                            f"Failed to fetch full text after retries {url}: {e}"
                        )
            return None
    ...
    async def batch_crawl(self, items: list):
        ...
        async with httpx.AsyncClient(
            limits=self.client_limits, follow_redirects=True
        ) as client:
            tasks = []
            for item in items:
            ...
            logger.info(
                f"✅ 全文提取任务完成: {processed_count}/{len(items)} 成功获取全文。"
            )
        return items
