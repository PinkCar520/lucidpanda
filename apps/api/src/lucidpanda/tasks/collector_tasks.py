...
def _get_feed_state(feed_name: str) -> dict[str, Any]:
    ...
    state = cast(dict[str, Any], r.hgetall(key))
    ...
    if new_items == 0:
        if state["consecutive_empty_count"] >= EMPTY_THRESHOLD:
            ...
            if new_interval != old_interval:
                logger.info(
                    f"📈 [{feed_name}] 连续{state['consecutive_empty_count']}次空，间隔调整为{new_interval}s"
                )
    else:
        state["consecutive_empty_count"] = 0
        ...
        if new_interval != old_interval:
            logger.info(
                f"📉 [{feed_name}] 采集到{new_items}条，间隔调整为{new_interval}s"
            )
...
async def fetch_single_feed_task(
    feed_name: str, feed_url: str, category: str
) -> dict[str, Any]:
    db = IntelligenceDB()
            ...
            if saved > 0:
                try:
                    ...
                    r.publish(
                        ...
                        json.dumps({"type": "new_data", "count": saved}),
                    )
                    r.publish(
                        "lucidpanda:new_intelligence",
                        json.dumps({"type": "new_data", "count": saved}),
                    )
...
async def fetch_all_feeds() -> dict[str, Any]:
    ...
    for feed in TIER1_FEEDS_CONFIG:
        ...
        if should_fetch:
            tasks.append(
                await fetch_single_feed_task.kiq(
    ...
    return {"total_feeds": len(TIER1_FEEDS_CONFIG), "dispatched": len(tasks)}
