...
class RSSHubSource(BaseDataSource):
    ...
    async def _fetch_feed_async_wrapper(
        self,
        url: str,
        category: str,
        name: str = "unknown"
        ...
        Returns:
            情报列表
        """
        config = {"name": name, "url": url, "category": category}
        items, _ = await self._fetch_feed_async_simple(url, category, name)
        return items
    
    async def _fetch_feed_async_simple(
        self,
        url: str,
        category: str,
        name: str
        ...
        try:
            async with httpx.AsyncClient(headers=headers, timeout=15.0) as client:
                ...
                for entry in feed.entries:
                    ...
                    # 检查是否已存在
                    if await asyncio.to_thread(self.db.source_id_exists, item_id):
                        continue
                ...
                status["status"] = "ok" if items else "ok_empty"
                return items, status
