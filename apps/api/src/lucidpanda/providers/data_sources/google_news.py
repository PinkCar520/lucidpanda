        ...
        try:
            ...
            def format_date_for_rss(date_str):
                ...
                try:
                    ...
                    # Assume already formatted or handled by dateparser
                    return date_str
            ...
            for attempt in range(3):
                try:
                    ...
                    if response.status_code == 200:
                        ...
                        logger.warning(
                            f"Google News RSS returned status {response.status_code}, attempt {attempt + 1}/3"
                        )
                except Exception as e:
                    logger.warning(
                        f"Attempt {attempt + 1}/3 failed to fetch Google News RSS: {e}"
                    )
                    if attempt < 2:
                        time.sleep(2)
            ...
            logger.info(
                f"Discovery: Found {len(new_items)} potential new items from Google News."
            )
            return new_items
