...
async def get_web_backtest_stats(
    ...
    try:
        ...
        # 6.1 Alpha Return Distribution (if available)
        query_alpha_dist = f"""
        {base_cte}
        SELECT floor(alpha_return / 0.5) * 0.5 as bin, COUNT(*) as count
        FROM deduplicated_events
        WHERE alpha_return IS NOT NULL
        GROUP BY bin ORDER BY bin ASC;
        """

        query_alpha_summary = f"""
        {base_cte}
        SELECT COUNT(*) as count,
               AVG(alpha_return) as avg_alpha,
               COUNT(CASE WHEN alpha_return > 0 THEN 1 END) as wins
        FROM deduplicated_events
        WHERE alpha_return IS NOT NULL;
        """
