...
def get_passkey_login_options(db: Session = Depends(get_db)):
    auth_service = AuthService(db)
...
def verify_passkey_login(
    request: Request, body: PasskeyAuthenticationVerify, db: Session = Depends(get_db)
):
    auth_service = AuthService(db)
    user = auth_service.verify_authentication_response(
        body.auth_data, body.state, ip_address=request.client.host if request.client else "0.0.0.0"
    )
    if not user:
...
def update_api_key(
    ...
    auth_service = AuthService(db)
    key = auth_service.update_api_key(
        str(current_user.id), key_id, **body.dict(exclude_unset=True)
    )
    if not key:
...
def revoke_api_key(
    key_id: str, db: Session = Depends(get_db), current_user=Depends(get_current_user)
):
    auth_service = AuthService(db)
...
def update_notification_preferences(
    ...
    auth_service = AuthService(db)
    return auth_service.update_notification_preferences(
        str(current_user.id), **body.dict(exclude_unset=True)
    )
...
def verify_2fa(
    ...
    auth_service = AuthService(db)
    success, message = auth_service.verify_and_enable_2fa(
        str(current_user.id), body.secret, body.code
    )
    if not success:
        raise HTTPException(status_code=400, detail=message)
    return {"message": message}
...
def request_phone_verification(
    ...
    auth_service = AuthService(db)
    success, message = auth_service.request_phone_verification(
        str(current_user.id), body.phone_number
    )
    if not success:
        raise HTTPException(status_code=400, detail=message)
    return {"message": message}
...
def verify_phone_binding(
    ...
    auth_service = AuthService(db)
    success, message = auth_service.verify_phone_binding(
        str(current_user.id), body.phone_number, body.code
    )
    if not success:
        raise HTTPException(status_code=400, detail=message)
    return {"message": message}
...
def unbind_phone(db: Session = Depends(get_db), current_user=Depends(get_current_user)):
    auth_service = AuthService(db)
...
def get_sessions(
    ...
    auth_service = AuthService(db)
    current_token_hash = (
        auth_service.hash_refresh_token(x_refresh_token) if x_refresh_token else None
    )
        ...
        result.append(
            {
                ...
                "is_current": current_token_hash == session.token_hash
...
def refresh_token(
    request: Request, refresh_in: RefreshTokenRequest, db: Session = Depends(get_db)
):
    auth_service = AuthService(db)
    try:
...
async def upload_avatar(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Could not process image: {str(e)}"
        ) from e
...
def change_password(
    ...
    success, message = auth_service.change_password(
        str(current_user.id), body.current_password, body.new_password
    )
    if not success:
        raise HTTPException(status_code=400, detail=message)
    return {"message": message}
...
def initiate_email_change(
    ...
    success, message = auth_service.initiate_email_change(
        str(current_user.id), body.current_password, body.new_email
    )
    if not success:
        raise HTTPException(status_code=400, detail=message)
    return {"message": message}
...
def verify_email_change(body: EmailChangeVerifyRequest, db: Session = Depends(get_db)):
    auth_service = AuthService(db)
