...
class FundEngine:
    ...
    def _infer_market_region_from_meta(self, fund_code:
        str, meta: dict = None, fallback_name: str = "") -> str:
        meta = meta or {}
    ...
    def _market_day_progress(self, region:
        str, now_utc: datetime | None = None) -> float:
        """
    ...
    def _calc_fee_drag_pct(self, fund_code:
        str, fund_meta: dict = None, fund_name: str = ""):
        """
    ...
    def _get_industry_map(self, stock_codes:
        list):
        """
    ...
    def calculate_realtime_valuation(self, fund_code):
        ...
        try:
            ...
            result = {
                ...
                "market_status": get_market_status(self._infer_market_region_from_meta(fund_code, single_meta, fund_name)),
    ...
    def calculate_batch_valuation(self, fund_codes:
        list, summary: bool = False):
        """
        ...
        def infer_market_region(fund_code:
            str) -> str:
        ...
        results = []
        datetime.now().astimezone().replace(tzinfo=None) # simple local time

        # Pre-fetch industry mappings (Skip if summary)
        industry_map = {}
        if not summary:
            all_stock_codes = []
            for _, holdings in all_holdings.items():
                if holdings:
                    for h in holdings:
                        s_code = h.get('stock_code') or h.get('code')
                        if s_code:
    ...
    def reconcile_official_valuations(self, target_date=None, fund_codes:
        list[str] | None = None):
        """
        ...
        unique_dates = sorted({d for d, c in pending_tasks}, reverse=True)
