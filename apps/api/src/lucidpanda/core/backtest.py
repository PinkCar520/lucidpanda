...
class BacktestEngine:
    ...
    def process_signal(self, signal_direction: str) -> bool:
        ...
        if signal_direction == "Long":
            if self.current_position in [None, "Short"]:
                logger.info(
                    f"➡️ 新信号：Long. 当前持仓：{self.current_position}. 开多仓。"
                )
            ...
            if self.current_position in [None, "Long"]:
                logger.info(
                    f"➡️ 新信号：Short. 当前持仓：{self.current_position}. 开空仓。"
                )
    ...
    def _fetch_precise_hist(self) -> pd.DataFrame:
        ...
        try:
            ...
            if data and isinstance(data, dict):
                ...
                if points:
                    ...
                    logger.info(
                        f"📈 成功从 Sina 获取 {len(df)} 条 1m 精度数据 (XAUUSD 美元/盎司)"
                    )
        ...
        try:
            ...
            if data and "data" in data and data["data"]["klines"]:
                ...
                df["Close"] = pd.to_numeric(df["close"])
                logger.info(
                    f"📈 成功从 EastMoney 获取 {len(df)} 条 15m 精度数据 (XAUUSD 美元/盎司)"
                )
                return df.set_index("timestamp")[["Close"]]
        except Exception as e:
    ...
    def sync_outcomes(self):
        ...
        if (
            now - self.last_sync_attempt
        ).total_seconds() < self.sync_cooldown_minutes * 60:
            return
        ...
        logger.info(
            f"✅ 高精度同步完成：成功处理 {success_count}/{len(ready_records)} 条"
        )
