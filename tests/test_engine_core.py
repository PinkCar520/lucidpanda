...
def mock_dependencies(mocker):
    ...
    mock_dedup = mocker.patch('src.alphasignal.core.engine.NewsDeduplicator')
    ...
    mock_bt    = mocker.patch('src.alphasignal.core.engine.BacktestEngine')
...
def test_run_once_no_pending(engine):
    ...
    engine.run_once()
    engine.primary_llm.analyze_async.assert_not_called()
...
def test_run_once_with_pending_item(engine):
    ...
    engine.primary_llm.analyze_async = AsyncMock(return_value=analysis)
