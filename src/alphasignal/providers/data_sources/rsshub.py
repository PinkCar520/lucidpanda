    ...
    async def fetch_async(self) -> list | None:
        ...
        if self.log_each_source:
            for s in source_statuses:
                if s["status"] == "failed":
                    logger.warning(
                        f"❌ [RSS:{s['category']}] {s['name']} | 失败 | {s['reason']} | "
                        f"elapsed={s['elapsed_ms']}ms | {s['url']}"
                    )
                else:
                    reason_suffix = f" | reason={s['reason']}" if s['reason'] else ""
                    logger.info(
                        ...
                        f"elapsed={s['elapsed_ms']}ms{reason_suffix}"
                    )
