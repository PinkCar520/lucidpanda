...
# 构建并推送镜像
docker build -t pincar/alphasignal-backend:latest .
docker push pincar/alphasignal-backend:latest
```
...
```bash
# 登录 DB 确认新字段存在
docker exec -it alphasignal_db psql -U alphasignal -d alphasignal -c \
...
docker exec alphasignal_db psql -U alphasignal -d alphasignal -c "
...
```bash
# 在服务器容器内验证新模块可正确导入
docker exec alphasignal_worker python -c "
