...
| 文件路径 | 说明 | 状态 |
|---------|------|------|
| `src/lucidpanda/api/v1/routers/watchlist_v2.py` | 完整 v2 API 路由（分组、同步、批量操作） | ✅ 完成 |
| `src/lucidpanda/api/v1/main.py` | 注册新路由 | ✅ 完成 |
...
| `mobile/ios/LucidPanda/Data/Cache/WatchlistCache.swift` | SwiftData 本地缓存管理器 | ✅ 完成 |
| `mobile/ios/LucidPanda/Core/Sync/WatchlistSyncEngine.swift` | 同步引擎（SSE、离线队列、网络监控） | ✅ 完成 |
| `mobile/ios/LucidPanda/Features/Funds/FundViewModel.swift` | 完整 ViewModel（CRUD、分组、排序） | ✅ 完成 |
| `mobile/ios/LucidPanda/Features/Funds/FundDashboardView.swift` | 完整 UI 视图（分组筛选、编辑模式等） | ✅ 完成 |
...
# 或直接重启
python -m uvicorn src.lucidpanda.main:app --reload
```
...
```bash
# 打开 Xcode
open mobile/ios/LucidPanda/LucidPanda.xcodeproj
...
- [API 设计](../../src/lucidpanda/api/v1/routers/watchlist_v2.py)
