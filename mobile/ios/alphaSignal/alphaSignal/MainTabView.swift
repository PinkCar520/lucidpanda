...
struct MainTabView: View {
    ...
    var body: some View {
        TabView(selection: $selectedTab) {
            // Tab 1: 信号 (Dashboard)
            Tab("app.tab.intelligence", systemImage: "antenna.radiowaves.left.and.right", value: .intelligence) {
                MainDashboardView()
            }
            
            // Tab 2: 基金 (Watchlist)
            Tab("app.tab.funds", systemImage: "chart.line.uptrend.xyaxis", value: .funds) {
                FundDashboardView()
            }
            
            // Tab 3: 回测 (Strategy)
            Tab("app.tab.backtest", systemImage: "chart.bar.doc.horizontal.fill", value: .backtest) {
                BacktestView()
            }
