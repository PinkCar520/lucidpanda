...
def update_pinyin():
    ...
    with engine.connect() as conn:
        result = conn.execute(text("SELECT stock_code, stock_name FROM stock_metadata"))
        ...
        for stock in stocks:
            ...
            if shorthand:
                conn.execute(
                    text("UPDATE stock_metadata SET pinyin_shorthand = :p WHERE stock_code = :c"),
                    {"p": shorthand, "c": code_str}
                )
            count += 1
            if count % 1000 == 0:
                conn.commit()
